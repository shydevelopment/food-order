...
export default function ManageRolesPage() {
  ...
  const getRoleLabel = (role: string | null) => {
    return getAccountRoleMeta(role)?.thaiLabel || 'User';
  };
        ...
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm ${isClosing ? 'animate-backdrop-out' : 'animate-backdrop-in'}`}>
          ...
          <div className={`max-h-[92vh] w-full max-w-md overflow-y-auto  border border-neutral-800 rounded-xl p-4 shadow-2xl sm:p-6 ${isClosing ? 'animate-content-out' : 'animate-content-in'}`}>
            ...
            <form onSubmit={handleSaveChanges} className="space-y-4">
              ...
              <div className=" p-3 rounded-lg border border-neutral-800/60 space-y-1">
                ...
                <p className="text-xs text-neutral-400"><span className="text-neutral-500 font-bold">ชื่อผู้ใช้:</span> @{selectedUser?.username}</p>
              ...
              {/* Dropdown เลือกบทบาท */}
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">เลือก Role ใหม่</label>
                <div className="mb-2 grid gap-2">
                  {ACCOUNT_ROLES.map((role) => (
                    <div key={role.value} className="rounded-lg border border-neutral-800  p-3">
                ...
                <div className="relative">
                  <select
                    ...
                    >
                    <option value="" disabled className="text-neutral-600">-- กรุณาเลือก Role --</option>
                    {ACCOUNT_ROLES.map((r) => (
