...
const activeOrderStatuses = ['pending', 'preparing', 'delivering']

const statusLabels: Record<string, string> = {
  pending: 'รอร้านรับออเดอร์',
  preparing: 'ร้านกำลังเตรียมอาหาร',
  delivering: 'อาหารพร้อมให้มารับ',
}

const statusTones: Record<string, NotificationFeedItem['tone']> = {
  pending: 'orange',
  preparing: 'sky',
  delivering: 'emerald',
}
...
const getRestaurantIdsForUser = async (
  ...
  const [{ data: ownerRestaurants }, { data: restaurantMembers }] = await Promise.all([
    supabaseAdmin
      .from('restaurants')
      .select('id')
  ...
  const ownerRestaurantIds = ((ownerRestaurants || []) as Array<{ id: string }>).map((restaurant) => restaurant.id)
  const memberRestaurantIds = ((restaurantMembers || []) as RestaurantMemberRow[]).map((member) => member.restaurant_id)

  return Array.from(new Set([...ownerRestaurantIds, ...memberRestaurantIds]))
}

const getRestaurantsById = async (supabaseAdmin: SupabaseClient, restaurantIds: string[]) => {
  ...
  return new Map(((data || []) as RestaurantRow[]).map((restaurant) => [restaurant.id, restaurant]))
}
...
const getOrderNotifications = async (
  ...
  let ordersQuery = supabaseAdmin
    .from('orders')
    .select('id, order_no, user_id, restaurant_id, total_price, status, pickup_time, created_at')
    .in('status', activeOrderStatuses)
    .order('created_at', { ascending: false })
    .limit(30)

  if (profile.role === 'restaurant') {
    ordersQuery = allowedRestaurantIds && allowedRestaurantIds.length > 0
  ...
  const customerIds = Array.from(new Set(orderRows.map((order) => order.user_id)))
  ...
  return orderRows.map((order): NotificationFeedItem => {
    ...
    const restaurantName = restaurantsById.get(order.restaurant_id)?.name || 'ไม่พบชื่อร้าน'
    const customer = customersById.get(order.user_id)
    const customerName = customer?.full_name || customer?.username || customer?.email || 'ลูกค้า'

    return {
      id: `order-${order.id}`,
      type: 'order',
      title: `Order #${order.order_no || order.id.slice(0, 8)}`,
      detail: `${statusLabels[status] || 'ออเดอร์กำลังดำเนินการ'} · ${isStaffView ? `ลูกค้า ${customerName} · ` : ''}${restaurantName}${order.pickup_time ? ` · ${order.pickup_time.slice(0, 5)} น.` : ''}`,
      href: isStaffView ? `/admin/orders?restaurantId=${order.restaurant_id}` : `/trackorderPage?order=${order.id}`,
      created_at: order.created_at,
      tone: statusTones[status] || 'orange',
    }
  })
}
...
const getChatNotifications = async (
  ...
  if (profile.role === 'restaurant') {
    conversationsQuery = allowedRestaurantIds && allowedRestaurantIds.length > 0
  ...
  const [{ data: restaurants }, { data: senders }] = await Promise.all([
    restaurantIds.length > 0
      ...
      : Promise.resolve({ data: [] }),
    senderIds.length > 0
      ? supabaseAdmin.from('profiles').select('id, full_name, username').in('id', senderIds)
      : Promise.resolve({ data: [] }),
  ])
  ...
  return messageRows.map((message): NotificationFeedItem => {
    ...
    const restaurantName = conversation ? restaurantsById.get(conversation.restaurant_id)?.name || null : null
...
const syncNotificationsToSupabase = async (
  ...
  if (error) {
    if (error.message.includes('schema cache') || error.message.includes('notifications')) {
      throw new Error('ฐานข้อมูลยังไม่มีตาราง notifications กรุณารันไฟล์ supabase/notifications.sql ใน Supabase SQL Editor ก่อน')
    }
...
const readNotificationsFromSupabase = async (
  ...
  const { data, error } = await supabaseAdmin
    .from('notifications')
    .select('id, item_key, type, title, detail, href, tone, is_read, source_created_at')
    .eq('user_id', userId)
  ...
  return ((data || []) as NotificationRow[]).map((item): NotificationFeedItem => ({
...
export const getNotificationFeed = async (
  ...
  const allowedRestaurantIds = await getRestaurantIdsForUser(supabaseAdmin, userId, profile)
  ...
  const derivedItems = [...orderItems, ...chatItems]
  ...
  return {
    profile,
    items: await readNotificationsFromSupabase(supabaseAdmin, userId),
  }
}
