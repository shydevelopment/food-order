...
export default function EditProfileForm({ profile, email, updateAction }: EditProfileFormProps) {
  ...
  // ฟังก์ชันสลับแท็บ / เปิด-ปิด สไลด์บนมือถือ
  const toggleTab = (tab: 'profile' | 'avatar' | 'password') => {
    setErrorMessage(null)
    setSuccessMessage(null)
    // ถ้าย้ำแท็บเดิมบนมือถือให้พับเก็บ (null) ถ้ากดแท็บใหม่ให้เปิดสไลด์
  ...
  return (
    <div className="w-full max-w-5xl mx-auto text-white p-2">
      ...
      {/* Header Profile แสดงรูป + ชื่อผู้ใช้เบื้องต้น */}
      ...
      {/* ========================================================
          📱 1. LAYOUT สำหรับจอมือถือ (ACCORDION SLIDE DOWN)
      ======================================================== */}
      <div className="flex flex-col gap-3 md:hidden">
        ...
        <div className="bg-neutral-900 border border-neutral-800/80 rounded-2xl overflow-hidden shadow-lg">
          <button
            ...
            className="w-full p-4 flex items-center justify-between text-left font-bold text-sm bg-neutral-900 hover:bg-neutral-800/60 active:scale-[0.99] transition-all cursor-pointer"
          >
            <span className="flex items-center gap-3">
              <span className="text-base">👤</span>
              <span className={activeTab === 'profile' ? 'text-orange-500' : 'text-neutral-300'}>Edit Profile</span>
            </span>
            <span className={`text-xs text-neutral-400 transition-transform duration-300 ${activeTab === 'profile' ? 'rotate-180 text-orange-500' : ''}`}>
              ▼
            </span>
          </button>
          ...
          <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-in-out ${activeTab === 'profile' ? 'grid-rows-[1fr] opacity-100 border-t border-neutral-800/60' : 'grid-rows-[0fr] opacity-0'}`}>
            <div className="overflow-hidden bg-neutral-950/50">
              <form onSubmit={handleSubmitProfile} className="p-4 flex flex-col gap-4">
                ...
                <button type="submit" disabled={isPending} className="w-full bg-orange-500 hover:bg-orange-400 active:scale-95 rounded-xl py-2.5 text-black font-bold shadow-lg text-sm transition-all mt-2">
        ...
        <div className="bg-neutral-900 border border-neutral-800/80 rounded-2xl overflow-hidden shadow-lg">
          ...
          >
            ...
            <span className={`text-xs text-neutral-400 transition-transform duration-300 ${activeTab === 'avatar' ? 'rotate-180 text-orange-500' : ''}`}>
              ▼
            </span>
          </button>
            ...
            <div className="overflow-hidden bg-neutral-950/50">
              <div className="p-6 flex flex-col items-center gap-4 text-center">
                <div 
                  ...
                  onClick={() => document.getElementById('avatar-file-mobile')?.click()}
                >
                  {avatarUrl ? (
                ...
                <button 
                  ...
                  disabled={isPending} 
                  onClick={() => document.getElementById('avatar-file-mobile')?.click()} 
                  className="w-full bg-orange-500 hover:bg-orange-400 text-black active:scale-95 text-xs font-bold py-2.5 rounded-xl transition-all cursor-pointer shadow-lg disabled:bg-neutral-800"
                >
        ...
        <div className="bg-neutral-900 border border-neutral-800/80 rounded-2xl overflow-hidden shadow-lg">
          ...
          >
            ...
            <span className={`text-xs text-neutral-400 transition-transform duration-300 ${activeTab === 'password' ? 'rotate-180 text-orange-500' : ''}`}>
              ▼
            </span>
          </button>
          ...
          <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-in-out ${activeTab === 'password' ? 'grid-rows-[1fr] opacity-100 border-t border-neutral-800/60' : 'grid-rows-[0fr] opacity-0'}`}>
            <div className="overflow-hidden bg-neutral-950/50">
              <form onSubmit={handleSubmitPassword} className="p-4 flex flex-col gap-4">
                ...
                <button type="submit" disabled={isPending} className="w-full bg-orange-500 hover:bg-orange-400 active:scale-95 rounded-xl py-2.5 text-black font-bold shadow-lg text-sm transition-all mt-2 disabled:bg-neutral-800">
      ...
      {/* ========================================================
          💻 2. LAYOUT สำหรับจอคอมพิวเตอร์ DESKTOP (2-COLUMNS SIDEBAR)
      ======================================================== */}
      <div className="hidden md:flex gap-6 items-start">
        ...
        <div className="w-64 bg-neutral-900 border border-neutral-800/80 rounded-2xl p-4 flex flex-col gap-2 shrink-0 shadow-lg">
          <button 
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left font-bold text-sm transition-all active:scale-95 cursor-pointer ${activeTab === 'profile' ? 'bg-orange-500 text-black shadow-md shadow-orange-500/10' : 'text-neutral-400 hover:bg-neutral-800/60 hover:text-neutral-200'}`}
          >
            <span>👤</span> Profile
          </button>
          <button 
            type="button" 
            onClick={() => { setActiveTab('avatar'); setErrorMessage(null); setSuccessMessage(null); }} 
            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left font-bold text-sm transition-all active:scale-95 cursor-pointer ${activeTab === 'avatar' ? 'bg-orange-500 text-black shadow-md shadow-orange-500/10' : 'text-neutral-400 hover:bg-neutral-800/60 hover:text-neutral-200'}`}
          >
            <span>🖼️</span> Avatar
          </button>
          <button 
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left font-bold text-sm transition-all active:scale-95 cursor-pointer ${activeTab === 'password' ? 'bg-orange-500 text-black shadow-md shadow-orange-500/10' : 'text-neutral-400 hover:bg-neutral-800/60 hover:text-neutral-200'}`}
          >
            <span>🔒</span> Password
          </button>
        </div>
            ...
            <div className="animate-in fade-in duration-200">
              ...
              <form onSubmit={handleSubmitProfile} className="p-6 flex flex-col gap-5">
                <div className="space-y-4 max-w-xl">
                  ...
                  <div className="grid grid-cols-3 items-center gap-4">
                    <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider text-right">Email Address</label>
                    <input className="col-span-2 rounded-xl px-4 py-2 bg-neutral-950 border border-neutral-900/40 text-neutral-500 cursor-not-allowed select-none text-sm" type="email" value={email || ''} disabled />
                  </div>
                </div>
                <div className="flex items-center justify-end gap-3 max-w-xl border-t border-neutral-800/80 pt-4 mt-2">
                  <Link href="/" className="border border-neutral-800 rounded-xl px-5 py-2 text-neutral-400 hover:bg-neutral-950 hover:text-orange-500 transition-all text-sm active:scale-95">Cancel</Link>
