...
const nextActions = [
  ...
  { status: 'delivering', label: 'พร้อมให้มารับอาหาร', visibleFrom: ['preparing'] },
    ...
    try {
      const res = await fetch('/api/admin/orders/status', {
        ...
        body: JSON.stringify({
          orderId,
          status: nextStatus,
          cancellationReason: nextStatus === 'cancelled' ? cleanedReason : undefined,
        }),
      })
    ...
    <>
      ...
      {showCancelDialog && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/75 px-4 py-6 text-white backdrop-blur-sm food-alert-overlay">
          <div className="w-full max-w-xl rounded-2xl border border-neutral-800 bg-neutral-900 p-5 shadow-2xl shadow-black/60 food-alert-panel">
            ...
            <p className="mt-4 text-center text-xs font-black uppercase tracking-wide text-red-400">ยกเลิกออเดอร์</p>
            <h2 className="mt-2 text-center text-2xl font-black text-white">กรอกเหตุผลให้ลูกค้าทราบ</h2>
            ...
            <textarea
              ...
              onChange={(event) => setCancellationReason(event.target.value.slice(0, 300))}
              placeholder="เช่น วัตถุดิบหมด, ร้านปิดฉุกเฉิน, ทำออเดอร์นี้ไม่ทัน"
              className="mt-2 w-full resize-none rounded-xl border border-neutral-800 bg-neutral-950 px-3 py-3 text-sm text-white placeholder-neutral-600 outline-none transition focus:border-red-500"
            />
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]">
              <button
                ...
                className="rounded-xl bg-red-500 px-5 py-3 text-sm font-black text-white transition hover:bg-red-400 disabled:cursor-not-allowed disabled:bg-neutral-800 disabled:text-neutral-500"
              >
                {updatingStatus === 'cancelled' ? 'กำลังยกเลิก...' : 'ยืนยันยกเลิกออเดอร์'}
              </button>
              <button
                type="button"
                disabled={Boolean(updatingStatus)}
                ...
                className="rounded-xl border border-neutral-700 bg-neutral-950 px-5 py-3 text-sm font-bold text-neutral-300 transition hover:bg-neutral-800 hover:text-white disabled:opacity-50"
              >
                กลับ
              </button>
            </div>
